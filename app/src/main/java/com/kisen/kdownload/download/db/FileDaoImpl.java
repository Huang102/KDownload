...
public class FileDaoImpl implements FileDao {
    ...
    public void deleteFile(String url, int fileId) {
        SQLiteDatabase db = mDBHelper.getReadableDatabase();
        if (db.isOpen()) {
            db.delete("file_info", "url = ? and file_id = ?", new String[]{url, fileId + ""});
            db.close();
        }
    }
    ...
    public void updateFile(String url, int fileId, int finished) {
        SQLiteDatabase db = mDBHelper.getReadableDatabase();
        String sql = "update file_info set finished = ? where url = ? and file_id = ?";
    ...
    public boolean isExists(String url) {
        SQLiteDatabase db = mDBHelper.getReadableDatabase();
        Cursor cursor = db.query("file_info", null, "url = ?", new String[]{url},
                null, null, null);
